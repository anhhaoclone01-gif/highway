...
class AbstractEnv(gym.Env):
    ...
    def _info(self, obs: Observation, action: Action | None = None) -> dict:
        ...
        # Append danger metrics if environment provides them (useful for logging/tests)
        try:
            if hasattr(self, "_compute_danger_metrics"):
                info["danger"] = self._compute_danger_metrics()
        except Exception:
            # Do not break if danger computation fails
            pass
